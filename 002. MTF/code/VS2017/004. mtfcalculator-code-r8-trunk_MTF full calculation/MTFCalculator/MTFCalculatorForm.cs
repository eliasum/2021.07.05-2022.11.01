    ...
    public partial class MTFCalculatorForm : Form
    {
        ...
        //private void Compute(Bitmap bitmap)
        {
            ...
            */
            ...
            if (File.Exists(filePath))
            {
                ...
                for (int i = 0; i < text.Length; i++)
                {
                    ...
                    listESF1.Add(steps_quickmtf[i], ESF1[i]);
                    listESF2.Add(steps_quickmtf[i], ESF2[i]);
                    listESF3.Add(steps_quickmtf[i], ESF3[i]);
                    listESF4.Add(steps_quickmtf[i], ESF4[i]);
                }
            ...
            */
            ...
            /*
            // сглаживание графиков
            if (croppedBitmap.Width <= 100) Smooth(difference, 2);
            else
            if (croppedBitmap.Width <= 300) Smooth(difference, 11);
            */
            ...
            // нормализация графика LSF относительно вертикальной оси /////////////////////////////////////////////////////////////
            // без применения Antialiasing()
            Normalize(difference);

            // с применением Antialiasing()
            Normalize(differenceOut);
            /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            if (File.Exists(filePathLSF))
            {
                ...
                {
                    // добавим в списки точку
                    listLSF1.Add(steps_quickmtf[i], LSF1[i]);
                    listLSF2.Add(steps_quickmtf[i], LSF2[i]);
                    listLSF3.Add(steps_quickmtf[i], LSF3[i]);
                    listLSF4.Add(steps_quickmtf[i], LSF4[i]);
                }
            ...
            */
            ...
            using (var SW_MTF_array = new StreamWriter(@"csv\" + output_string + @"_VS2017_QuickMTF_MTF.csv", false, Encoding.Default))
            {
                for (int i = 0; i < MTF_Length; i++)
                    SW_MTF_array.WriteLine(stepsMTF[i].ToString() + ";" + myGraphPanelMTF_Y[i].ToString());
            }

            // запись в файл исходных данных MTF
            using (var SW_MTF_array = new StreamWriter(@"csv\" + output_string + @"_VS2017_MTF.csv", false, Encoding.Default))
            {
                for (int i = 0; i < MTF_Length; i++)
                    SW_MTF_array.WriteLine(i.ToString() + ";" + myGraphPanelMTF_Y[i].ToString());
            }

            // считать данные MTF из файла, сформированного quickmtf для сравнения на одном графике______________________________

            /*
                проверка существования папок 'QuickMTF_output\MTF' и 'SFRCalculation_output\MTF'
                данных MTF программ QuickMTF и SFRCalculation
            */
            ...
            // путь к файлу данных MTF, имя которого аналогично имени тестируемой картинки
            ...
            if (File.Exists(filePathMTF)&& File.Exists(filePathSFR))
            {
                // считать файл построчно в массив строк
