...
CREATE TABLE table_Glossary (
	...
	FOREIGN KEY (tablename) REFERENCES tablenames (tablename)
);
...
CREATE TABLE tablenames (
    key integer NOT NULL GENERATED ALWAYS AS IDENTITY ( INCREMENT 1 START 1 MINVALUE 1 MAXVALUE 2147483647 CACHE 1 ),
    tablename character(15) NOT NULL,
    CONSTRAINT "tablenames_key" PRIMARY KEY (key),
);
...
SELECT create_hypertable3(
  'table_Decimal', 'tstamp',
  chunk_time_interval => INTERVAL '1 day'
);

INSERT INTO table_Decimal
  SELECT
    tstamp || (random()*30)::INT
  FROM
    generate_series(
...
SELECT create_hypertable4(
  'table_String', 'tstamp',
  chunk_time_interval => INTERVAL '1 day'
);

INSERT INTO table_String
  SELECT
    tstamp || (random()*30)::INT
  FROM
    generate_series(
