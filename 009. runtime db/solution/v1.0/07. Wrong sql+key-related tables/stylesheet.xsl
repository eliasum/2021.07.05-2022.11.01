  ...
  <xsl:template match="node()[@format]">
    ...
    <xsl:choose>
      <xsl:when test="starts-with(@format, 'N')">
        <xsl:value-of select="concat(local-name(), $apostrophe, ', ', 'NULL, ', $allowance, ', ', $apostrophe, 'table_Number', $apostrophe, '),')"/>
      </xsl:when>
      <xsl:when test="starts-with(@format, 'F')">
        <xsl:value-of select="concat(local-name(), $apostrophe, ', ', 'NULL, ', $allowance, ', ', $apostrophe, 'table_Float', $apostrophe, '),')"/>
      </xsl:when>
      <xsl:when test="starts-with(@format, 'D')">
        <xsl:value-of select="concat(local-name(), $apostrophe, ', ', 'NULL, ', $allowance, ', ', $apostrophe, 'table_Decimal', $apostrophe, '),')"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="concat(local-name(), $apostrophe, ', ', 'NULL, ', $allowance, ', ', $apostrophe, 'table_String', $apostrophe, '),')"/>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:text>&#10;</xsl:text>
  </xsl:template>
