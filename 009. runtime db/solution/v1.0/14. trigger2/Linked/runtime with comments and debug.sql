...
COMMENT ON TABLE table_glossary IS '—ловарь дл€ индефикации переменных';
COMMENT ON COLUMN table_glossary.key IS '”никальный ключ переменной дл€ св€зи с друними таблицами';
COMMENT ON COLUMN table_glossary.configuration IS 'XPath - путь к переменной в XML-конфигурационном файле дл€ ѕќ';
COMMENT ON COLUMN table_glossary.communication IS 'XPath - путь к переменной в XML-конфигурационном файле дл€ коммуникаций';
COMMENT ON COLUMN table_glossary.allowance IS 'ƒопуск на изменение переменной';
COMMENT ON COLUMN table_glossary.tablename IS '»м€ таблицы в которой хран€тс€ значени€ переменной';
/*-------------------------------------------------------------------------------------------------*/
...
DECLARE 
    ...
    _Path VARCHAR; 			--переменна€ дл€ входного параметра xpath
	_Table TEXT;  			--им€ актуальной таблицы
	_HyperTable TEXT;		--им€ соответствующей гипертаблицы
	_Allowance FLOAT;		--допуск, 3-е поле таблицы table_glossary
	_ValueOld FLOAT;		--последнее значени€ 3-го пол€ val из актуальной таблицы
	_Count INTEGER;			--число строк в гипертаблице
	_1P FLOAT;
	_Delta FLOAT;
	_DeltaP FLOAT;
...
BEGIN
	...
	/*ѕолучение имени дочерней таблицы из XPath таблицы table_glossary*/
	...
	/*ѕолучение allowance (допуска) из XPath таблицы table_glossary*/
	IF (SELECT EXISTS(SELECT 1 from table_glossary WHERE configuration = _Path)) THEN
	...
	/*ѕолучение последнего значени€ val из актуальной таблицы*/
	EXECUTE FORMAT('SELECT val FROM %I ORDER BY key DESC LIMIT 1', _Table) INTO _ValueOld;
	
	/*ѕолучение разницы в % между старым и новым значени€ми*/
	_1P := _ValueOld / 100; 
	_Delta := ABS(_ValueOld - valnew);
	_DeltaP := _Delta / _1P;
			
	/*ѕолучение имени гипертаблицы в зависимости от имени актуальной таблицы*/
	CASE 
	...
	/*„исло строк в гипертаблице*/
	EXECUTE FORMAT('SELECT COUNT(*) FROM %I', _HyperTable) INTO _Count;
																	
	/*≈сли в допуске*/
	IF(_DeltaP <= _Allowance) THEN
		/*≈сли число записей в гипертаблице не превышает 2*/
		IF(_Count < 2) THEN
			/*INSERT INTO number_actual(tstamp, val) VALUES(NOW(), valnew);*/	
			EXECUTE format('INSERT INTO %I(tstamp, val) VALUES(NOW(), %s);', _Table, valnew);
		ELSE
			/*UPDATE number_actual SET tstamp = NOW() WHERE key = (SELECT MAX(key) from number_actual);*/
...
BEGIN
	/*NEW.tablename - значение пол€ tablename таблицы table_glossary, 
	...
	/*—оздание актуальной и гипертаблицы, если они не существуют, 
	исход€ из значени€ пол€ tablename таблицы table_glossary*/	
	EXECUTE format('
