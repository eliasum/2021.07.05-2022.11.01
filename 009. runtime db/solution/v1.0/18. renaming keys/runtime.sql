...
BEGIN
	...
	IF(_Delta <= _Allowance) THEN
		...
		IF(_Count < 2) THEN
			EXECUTE FORMAT('INSERT INTO %I(fix, value) VALUES(NOW(), %s);', _Table, value);
		ELSE
...
BEGIN
	...
	IF (_Table IS NOT NULL) THEN
		RETURN QUERY EXECUTE FORMAT('SELECT * FROM %I WHERE %I.key = %s;', _Table, _Table, _Key);
	ELSE
...
RETURNS TABLE (rkey INTEGER, rconfig TEXT, rfix TIMESTAMPTZ, value_float FLOAT, value_integer INTEGER) AS $$
...
BEGIN
	...
	SELECT g.key AS key, g.configuration AS config, float_actual.fix AS fix, float_actual.value AS float_value, NULL AS integer_value
	FROM glossary AS g
	INNER JOIN float_actual ON g.key = float_actual.key 
	UNION
	SELECT g.key AS key, configuration AS config, integer_actual.fix AS fix, NULL AS float_value, integer_actual.value AS integer_value
	FROM glossary AS g
