...
-----------------------------------------------------------------------------------------------------
INSERT INTO glossary (configuration, communication, tablename) VALUES --</xsl:text>
    <xsl:apply-templates/>
    <xsl:text>;
-----------------------------------------------------------------------------------------------------
...
$$ LANGUAGE plpgsql;
  ...
  <xsl:template match="node()[@type]">
    ...
    <xsl:value-of select="concat(local-name(), '/@value', $apostrophe, ', ', 'NULL, ', $apostrophe, $type, $apostrophe, ')')"/>
    <xsl:text>&#10;</xsl:text>
  </xsl:template>
