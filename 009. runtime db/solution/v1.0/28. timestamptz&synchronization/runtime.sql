...
DECLARE 
	...
	_MaxFixArchive TIMESTAMPTZ;		--последний фикс гипертаблицы по ключу _Key
	_MaxFixSynchro TIMESTAMPTZ;		--последний фикс таблицы synchronization
...
BEGIN
	...
	--Вычислить последние фиксы в гипертаблице (по ключу) и таблице synchronization
	EXECUTE FORMAT('SELECT MAX(fix) FROM %I WHERE key = %s;', _HyperTable, _Key) INTO _MaxFixArchive;
	SELECT INTO _MaxFixSynchro (SELECT MAX(fix) FROM synchronization);	
	
	--Если после синхронизации не было записи в гипертаблицу, то сделать 2 записи
	IF(_MaxFixSynchro > _MaxFixArchive) THEN
		EXECUTE FORMAT('INSERT INTO %I(key, fix, value) VALUES(%s, NOW()::TIMESTAMP WITH TIME ZONE, %s);', _HyperTable, _Key, valueNew);
		EXECUTE FORMAT('INSERT INTO %I(key, fix, value) VALUES(%s, NOW()::TIMESTAMP WITH TIME ZONE + interval ''10 millisecond'', %s);', _HyperTable, _Key, valueNew);
	ELSE
	END IF;	
	...
	IF (_Existence) THEN
		EXECUTE FORMAT('UPDATE %I SET fix = NOW()::TIMESTAMP WITH TIME ZONE, value = %s WHERE key = %s;', _Table, valueNew, _Key);
	ELSE
		EXECUTE FORMAT('INSERT INTO %I(key, fix, value) VALUES(%s, NOW()::TIMESTAMP WITH TIME ZONE, %s);', _Table, _Key, valueNew);
	END IF;  
	...
	IF(_Delta IS NOT NULL AND _Delta <> 0) THEN
		...
		EXECUTE FORMAT('INSERT INTO %I(key, fix, value) VALUES(%s, NOW()::TIMESTAMP WITH TIME ZONE + interval ''10 millisecond'', %s);', _HyperTable, _Key, valueNew);
	ELSE
		EXECUTE FORMAT('UPDATE %I SET fix = NOW()::TIMESTAMP WITH TIME ZONE WHERE fix = (SELECT MAX(fix) FROM %I WHERE key = %s);', _HyperTable, _HyperTable, _Key);
	END IF;	
...
BEGIN
	...
	INSERT INTO synchronization (xpath, fix, status) VALUES (_xpath, NOW()::TIMESTAMP WITH TIME ZONE, _status);
...
BEGIN
	...
	IF (_Existence) THEN
		UPDATE order_actual SET fix = NOW()::TIMESTAMP WITH TIME ZONE, value = _value WHERE key = _Key;
	ELSE
		INSERT INTO order_actual(key, fix, value) VALUES(_Key, NOW()::TIMESTAMP WITH TIME ZONE, _value);
	END IF;  

	--вставить новую запись в таблицу order_archive
	INSERT INTO order_archive(key, fix, value) VALUES(_Key, NOW()::TIMESTAMP WITH TIME ZONE, _value);
...
RETURNS TABLE (_xpath TEXT, fix TIMESTAMPTZ, _value TEXT) AS $$
