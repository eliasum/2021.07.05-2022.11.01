    ...
    <xsl:variable name="dbname">
      <xsl:for-each select="descendant::*">
        <xsl:if test="Postgres">
          <xsl:value-of select="concat('_', translate(translate(substring-after(Postgres/Item/@key, '_'),'.','_'),'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'))"/>
        </xsl:if>
      </xsl:for-each>
    </xsl:variable>
...
--psql --username=postgres -d postgres -f C:\TEMP\</xsl:text>
