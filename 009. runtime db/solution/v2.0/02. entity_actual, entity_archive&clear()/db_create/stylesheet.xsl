...
$$ LANGUAGE plpgsql;
	...
	FOREIGN KEY (key) REFERENCES glossary(key));
-----------------------------------------------------------------------------------------------------
INSERT INTO glossary (communication, configuration, tablename, entity) VALUES --</xsl:text>
    <xsl:apply-templates/>
    <xsl:text>;
-----------------------------------------------------------------------------------------------------
...
$$ LANGUAGE plpgsql;
  ...
  <xsl:template match="node()[@type]">
    ...
    <xsl:value-of select="concat(local-name(), $apostrophe, ', NULL, ', $apostrophe, $type, $apostrophe, ', NULL', ')')"/>
    <xsl:text>&#10;</xsl:text>
  </xsl:template>
