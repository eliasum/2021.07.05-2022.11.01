...
$$ LANGUAGE plpgsql;
  ...
  <!--шаблон соответствует любому узлу, кроме корневого, с атрибутом type-->
  <xsl:template match="node()[@type]">
	<!--переменная, хранящая значение атрибута type-->
    ...
    <xsl:for-each select="ancestor::*">
	  ...
	  <!--распечатать значение переменной $element-->
      <xsl:value-of select="$element"/>
	  <!--если текущий узел это 'Configuration' или 'Item'-->
	...
	<!--дальнейшее заполнение записей столбцов таблицы glossary-->
