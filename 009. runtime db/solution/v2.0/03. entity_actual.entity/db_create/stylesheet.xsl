...
DECLARE 
	...
	_Existence_entity BOOLEAN;		--признак существования в таблице entity_actual записи с ключом _Key
...
BEGIN
	...
	EXECUTE FORMAT('SELECT EXISTS(SELECT 1 from entity_actual WHERE key = %s)', _Key) INTO _Existence_entity;
	...
	--entity_actual
	IF (_Existence_entity AND _Key &lt;&gt; 0) THEN
...
RETURNS RECORD AS $$
	...
	--Использование функции:
	--SELECT * FROM _refresh('XPath') AS (entity TEXT, key INTEGER, fix TIMESTAMP, value FLOAT); - возврат записи из float_actual
	--SELECT * FROM _refresh('XPath') AS (entity TEXT, key INTEGER, fix TIMESTAMP, value INTEGER); - возврат записи из integer_actual
...
BEGIN
	...
	IF (_Table IS NOT NULL) THEN
		EXECUTE FORMAT('
		SELECT entity_actual.entity, %I.key, %I.fix, %I.value
		FROM %I 
		INNER JOIN entity_actual ON %I.key = %s
...
BEGIN
	...
	SELECT g.configuration AS config, entity_actual.entity AS entity, float_actual.fix AS fix, float_actual.value AS value_float, NULL AS value_integer
	FROM glossary AS g
	INNER JOIN float_actual ON g.key = float_actual.key 
	INNER JOIN entity_actual ON g.key = entity_actual.key 
	UNION
	SELECT g.configuration AS config, entity_actual.entity AS entity, integer_actual.fix AS fix, NULL AS value_float, integer_actual.value AS value_integer
	FROM glossary AS g
...
$$ LANGUAGE plpgsql;
-----------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION _clear(xpath TEXT)
RETURNS VOID AS $$
...
BEGIN
	...
	IF (_Key &lt;&gt; 0 AND _Key IS NOT NULL) THEN
		...
		INSERT INTO order_archive(key, fix, value) SELECT * FROM order_actual WHERE KEY = _Key;
    END IF;  
