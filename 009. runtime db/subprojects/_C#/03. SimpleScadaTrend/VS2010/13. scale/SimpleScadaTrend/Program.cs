...
namespace SimpleScadaTrend
{
    ...
    public class Program
    {
        ...
        {
            /*
                если принят 1 аргумент - путь ко входному файлу, а также
            ...
            */
            if (args.Length == 1 && File.Exists(args[0]))
            {
                ...
                string[] Units = new string[nodes.Count * 2];
                ...
                if (nodes != null)
                {
                    ...
                    foreach (XmlElement xnode in nodes)
                    {
                        ...
                        if (node.Name == "Item")
                        {
                            ...
                            nodeName = FirstChildNode.Name;

                            /*
                                проверка имени первого дочернего узла чтобы соблюсти правильный порядок единиц измерения
                                в массиве Units[]
                            */
                            // если имя первого дочернего узла - Amperage
                            if (nodeName == "Amperage")
                            {
                                // значение атрибута @unit тега Amperage без символов '[' и ']'
                                Units[u] = FirstChildNode.SelectSingleNode("@unit").Value.Trim(new char[] { '[', ']' }); u++;

                                // значение атрибута @unit тега Voltage без символов '[' и ']'
                                Units[u] = node.SelectSingleNode("Voltage").SelectSingleNode("@unit").Value.Trim(new char[] { '[', ']' }); u++;
                            }
                            // иначе если имя первого дочернего узла - Voltage
                            else
                                if (nodeName == "Voltage")
                                {
                                    // значение атрибута @unit тега Voltage без символов '[' и ']'
                                    ...
                                    // значение атрибута @unit тега Amperage без символов '[' и ']'
                                    Units[u] = node.SelectSingleNode("Amperage").SelectSingleNode("@unit").Value.Trim(new char[] { '[', ']' }); u++;
                                }
                        }
                    }
                }
