            ...
            */
            ...
            if (File.Exists(configuration) && File.Exists(communication))
            {
                ...
                for (int i = 0; i < configurationXpaths.Length; i++)
                {
                    ...
                    configurationNodes[i] = configurationRoot.SelectSingleNode("//Configuration[@key='_036CE062.ControlWorkstation']/Scale/Communication/Item[@key='database']/Postgres/Item[@key='localhost:5432:_036CE062']/Equipment[@key='_036CE062']/Elegaz/Item[@key='1']");
                }

                /****************************************************************units*****************************************************************************/

                // единицы измерения трендов
                string[] Units = new string[communicationNodes.Count];
                ...
                string[] Minimum = new string[communicationNodes.Count];

                // максимальное значение переменной (шкалы) тренда
                string[] Maximum = new string[communicationNodes.Count];
                ...
                // контекст данных
                //configurationNodes = configurationRoot.SelectNodes("//Pen/*");
                ...
                if (configurationNodes != null)
                {
                    ...
                    */
                    ...
                    // обход всех узлов в элементах nodes
                    foreach (XmlElement xnode in configurationNodes)
                    {
