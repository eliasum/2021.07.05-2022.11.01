...
namespace ConsoleApplication1
{
    ...
    class Program
    {
        ...
        {
            /*
                если принят 1 аргумент - путь ко входному файлу _input.xml, а также
                что этот файл существует
            */
            if (args.Length == 1 && File.Exists(args[0]))
            {
                // переменная для нового Xml документа _input.xml
                XmlDocument xDoc = new XmlDocument();

                // загрузить в переменную Xml документ _input.xml из корневой директории
                xDoc.Load(args[0]);

                // получим корневой элемент файла _input.xml
                        ...
                        using (NpgsqlConnection connectionPostgr = new NpgsqlConnection(connPostgrStr))
                        {
                            ...
                            if (float_act_notNull == null)
                            {
                                ...
                                for (int i = 1; i < valuesPostgres.Length + 1; i++)
                                {
                                    // определить запрос
                                    using (NpgsqlCommand command = new NpgsqlCommand(
                                        "INSERT INTO public.float_actual(key, fix, value) VALUES(" + i + ", NOW(), " + rnd.NextDouble() * 60 + ");", connectionPostgr))
                                    {
                                        // выполнить запрос
                                        using (command.ExecuteReader()) { }
                                    }
