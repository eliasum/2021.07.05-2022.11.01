        ...
        static void Main(string[] args)
        {
            ...
            */
            ...
            if (File.Exists(args0))
            {
                ...
                /*
                    присвоить ip компа во внутр. сети с именем "036CE062", где стоит БД postgreSQL
                    элементу массива key[0] строки подключения к PostgreSQL
                */
                key[0] = IP_036CE062;

                // присвоить имя БД из файла config.xml переменной DB_NAME
                DB_NAME = key[2];
                ...
                // контекст данных
                XmlNodeList nodes = xRoot.SelectNodes("//Amperage | //Voltage | //Temperature | //Time | //Rate | //State");

                XmlNodeList nodesA = xRoot.SelectNodes("//Amperage");
                XmlNodeList nodesV = xRoot.SelectNodes("//Voltage");
                XmlNodeList nodesTmpr = xRoot.SelectNodes("//Temperature");
                XmlNodeList nodesTime = xRoot.SelectNodes("//Time");
                XmlNodeList nodesR = xRoot.SelectNodes("//Rate");
                XmlNodeList nodesState = xRoot.SelectNodes("//State");

                int cnt = nodesA.Count + nodesV.Count + nodesTmpr.Count + nodesTime.Count + nodesR.Count + nodesState.Count;
                int a = 0 + cnt;
                ...
                // параметр наличия данных в таблице bool_actual
                string bool_act_notNull = null;
                        ...
                        do
                        {
                                ...
                                */
                                ...
                                foreach (XmlElement xnode in nodes)
                                {
                                    ...
                                    if (node.Name == "Amperage" || node.Name == "Temperature" || node.Name == "Rate")
                                    {
                                        ...
                                        //using (NpgsqlCommand command = new NpgsqlCommand("select entity from public.value_actual where key = " + _float_, connectionPostgr))
                                        {
                                            // выполнить запрос
                                            using (NpgsqlDataReader reader = command.ExecuteReader())
                                            {
                                    ...
                                    if (node.Name == "Voltage" || node.Name == "Time")
                                    {
                                        ...
                                        //using (NpgsqlCommand command = new NpgsqlCommand("select entity from public.value_actual where key = " + _int_, connectionPostgr))
                                        {
                                            // выполнить запрос
                                            using (NpgsqlDataReader reader = command.ExecuteReader())
                                            {
                                    ...
                                    if (node.Name == "State")
                                    {
                                        ...
                                        //using (NpgsqlCommand command = new NpgsqlCommand("select entity from public.value_actual where key = " + _bool_, connectionPostgr))
                                        {
                                            // выполнить запрос
                                            using (NpgsqlDataReader reader = command.ExecuteReader())
                                            {
                            ...
                            try
                            {
                                ...
                                for (int i = 0; i < valuesPostgres.Length; i++)
                                {
                                    ...
                                    //command.CommandText = "INSERT [" + DB_NAME + "].[dbo].[trends_data] VALUES (" + i + ", CURRENT_TIMESTAMP, " + valuesPostgres[i].Replace(",", ".") + ", 1)";
