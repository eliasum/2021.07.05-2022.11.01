<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="*">
    <xsl:copy>
      ...
      <!--
      введение дополнительного условия, чтобы дочерние узлы тега Limit/Item
      не были так же дочерними тегами для узла, родительского для Limit
      -->
  ...
  <xsl:template name="generation">
    ...
    <Item key="{$key}">
      <Minimum>
        ...
        <!--
        запретить обрабатывать дочерние элементы текущего узла Minimum, 
        чтобы в выходном дереве у него не появился дочерний тег, например,
      ...
      <Maximum>
        ...
        <xsl:copy-of select="$template/*/@*[local-name() != 'const']"/>
        <!--
        запретить обрабатывать дочерние элементы текущего узла Maximum, 
        чтобы в выходном дереве у него не появился дочерний тег, например,
        <Widget select="2" readonly="false" />
        -->
        <!--xsl:apply-templates/-->
      ...
      <!--
      разрешить обрабатывать дочерние элементы текущего узла Item, 
      чтобы в выходном дереве у него появился дочерний тег, например,
      <Widget select="2" readonly="false" />
      -->
      <xsl:apply-templates/>
    </Item>
