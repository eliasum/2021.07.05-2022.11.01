<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="*">
    <xsl:copy>
      ...
      <!--
      обработка атрибута @Allowance, который теперь может присутствовать в тегах, дочерних
      тегу <Item key="Element"> в файле Dictionary.xml
      
      если у 'текущего тега из файла "Dictionary.xml"' с именем, которое совпадает со 
      значением из переменной $name (т.е. этот тег есть и во входном файле и в "Dictionary.xml"),
      есть аттрибут @Allowance 
      -->
      <xsl:if test="($dictionary//*[name(.) = $name])[@Allowance]">
        <!--создать у текущего атрибута дочерний тег Allowance-->
        <Allowance>
          <!--
          имеющий атрибут @const со значением, равным значению атрибута @Allowance
          текущего тега из файла "Dictionary.xml"
          -->
          <xsl:attribute name="const">
            <xsl:value-of select="$dictionary//*[name(.) = $name]/@Allowance"/>
          </xsl:attribute>
          <!--скопировать все атрибуты текущего тега из файла "Dictionary.xml", кроме
          атрибута @const-->
